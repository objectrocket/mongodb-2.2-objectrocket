...
namespace mongo {
    ...
    bool fileExceedsDiskSpaceQuota( const char *ns ) {
        if ( ! cmdLine.enforcediskquota ) {
            log() << "fileExceedsDiskSpaceQuota: not enforcing" << endl;
            return false;
        }
        unsigned long long quotabytes = (unsigned long long) cmdLine.diskquotasize * 1024 * 1024;

        log() << "diskquotasize total is " << quotabytes << endl;

        // get path handle for dbpath, check space remaining
        boost::filesystem::path db_path_handle = boost::filesystem::path(dbpath);
	try { 
	    boost::filesystem::space_info dbpath_space = boost::filesystem::space(db_path_handle); 
            if ( dbpath_space.available <= quotabytes ) {
                log() << "fileExceedsDiskSpaceQuota: hit quota limit for disk space" << endl;
                
                // return true if we're not in a "special" namespace, else false
                if (!NamespaceString::special( ns ) && NamespaceString( ns ).db != "local") {
                    return true;
                } else {
                    log() << "fileExceedsDiskSpaceQuota: we are over quota, but can't assert because we're in a special ns";
                }
            } else {
                log() << "fileExceedsDiskSpaceQuota: passed quota check for disk space - available is " << dbpath_space.available << endl;
                return false;
            }
	} catch (boost::filesystem::filesystem_error &e) { 
            log() << "fileExceedsDiskSpaceQuota: error occurred checking free disk space" << endl;
            return true;
	} 

        // looks good, return false
        return false;
    }
    ...
    MongoDataFile* Database::suitableFile( const char *ns, int sizeNeeded, bool preallocate, bool enforceQuota ) {
        ...
        for ( int i=numFiles()-1; i>=0; i-- ) {
            ...
            if ( f->getHeader()->unusedLength >= sizeNeeded ) {
                if ( fileIndexExceedsQuota( ns, i-1, enforceQuota ) || fileExceedsDiskSpaceQuota( ns ) ) // NOTE i-1 is the value used historically for this check.
                    ;
                else
                    return f;
        ...
        if ( fileIndexExceedsQuota( ns, numFiles(), enforceQuota ) || fileExceedsDiskSpaceQuota( ns ) ) {
            if ( cc().hasWrittenThisPass() ) {
                warning() << "quota exceeded, but can't assert, probably going over quota for: " << ns << endl;
            }
            else {
                uasserted(12501, "data limit exceeded - consider adding more shards via https://app.objectrocket.com");
            }
        }
