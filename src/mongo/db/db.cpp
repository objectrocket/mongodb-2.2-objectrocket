...
static int mongoDbMain(int argc, char* argv[]) {
    ...
    ("smalljournalfiles", "use small (128MB) journal files, instead of 1GB")
    ("uniformfilesize", "make all datafiles the same size")
    ("datafilesize", po::value<int>()->default_value(100), "set datafiles to this size (in megabytes), requires --uniformfilesize")
    ("enforcediskquota", "enforce free disk space quota to allocate new datafiles")
    ("diskquotasize", po::value<unsigned>()->default_value(100), "minimum amount of disk space required to allocate new datafiles, requires --enforcediskquota")
...

    {
        ...
        // ObjectRocket modifications for datafile sizing
        if (params.count("smalljournalfiles")) {
            verify( dur::DataLimitPerJournalFile >= 128 * 1024 * 1024 );
            dur::DataLimitPerJournalFile = 128 * 1024 * 1024;
        }
        if (params.count("uniformfilesize")) {
            cmdLine.uniformfilesize = true;
        }
        if (params.count("datafilesize")) {
            cmdLine.datafilesize = params["datafilesize"].as<int>();
        }
        if (params.count("diskquotasize")) {
            cmdLine.diskquotasize = params["diskquotasize"].as<unsigned>();
        }
        if (params.count("enforcediskquota")) {
            cmdLine.enforcediskquota = true;
        }
