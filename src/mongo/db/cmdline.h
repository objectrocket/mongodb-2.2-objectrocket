    ...
    */
    ...
    struct CmdLine {
        ...
        */
        ...
        // ObjectRocket mods for file sizing
        bool uniformfilesize;  // make all datafiles the same size
        int datafilesize;      // if uniformFileSize, make all datafiles this size
        bool smalljournalfiles;  // use small (128GB) journal files
        unsigned diskquotasize;      // minimum free space (MB) required to allocate new datafiles
        bool enforcediskquota;  // enforce a disk space limit to allocate new datafiles
    ...
    inline CmdLine::CmdLine() :
        ...
        uniformfilesize(false), datafilesize(100), smalljournalfiles(false),
        diskquotasize(100), enforcediskquota(false)
    {
        started = time(0);
