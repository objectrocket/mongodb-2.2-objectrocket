...
namespace mongo {
    ...
    */
    ...
    int MongoDataFile::maxSize() {
        ...
        else if ( cmdLine.uniformfilesize ) {
            return (cmdLine.datafilesize * 1024 * 1024);
        }
    ...
    int MongoDataFile::defaultSize( const char *filename ) const {
        ...
        if ( cmdLine.uniformfilesize && cmdLine.datafilesize ) {
            size = (cmdLine.datafilesize * 1024 * 1024);
        }
        return size;
    }
    ...
    bool MongoDataFile::openExisting( const char *filename ) {
        ...
        verify( sz % 4096 == 0 );
        if( sz < 64*1024*1024 && !cmdLine.smallfiles && !cmdLine.uniformfilesize) { 
            if( sz >= 16*1024*1024 && sz % (1024*1024) == 0 ) { 
                log() << "info openExisting file size " << sz << " but cmdLine.smallfiles=false and cmdLine.uniformfilesize=false" << endl;
            }
            else {
    ...
    void MongoDataFile::open( const char *filename, int minSize, bool preallocateOnly ) {
        ...
        verify( size >= 64*1024*1024 || cmdLine.smallfiles || cmdLine.uniformfilesize );
        verify( size % 4096 == 0 );
